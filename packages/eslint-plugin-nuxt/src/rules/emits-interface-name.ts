...
export const emitsInterfaceName = createRule({
  name: "emits-interface-name",
  meta: {
    type: "layout",
    docs: {
    ...
    messages: {
      "issue:invalid-interface-name":
        "Emits interface name '{{currentName}}' does not match expected pattern '{{expectedPattern}}'.",
    ...
    defaultOptions: [{ pattern: "^{{FileName}}Emits$" }],
  },
  create: (context) => {
    const options = context.options[0] || {};
    const rawPattern = options.pattern || "^{{FileName}}Emits$";
